...
function setSyncStatus(kind, message) {
  return sync.setSyncStatus(kind, message);
}

function persistGameState(reason = "State saved") {
  return sync.persistGameState(reason);
}

function queuePersist(reason) {
  return sync.queuePersist(reason);
}

function loadGameStateFromServer() {
  return sync.loadGameStateFromServer();
}

function pollForRemoteUpdates() {
  return sync.pollForRemoteUpdates();
}
const map = L.map('map', {
  zoomSnap: 0.25,
  zoomDelta: 0.5,
